...
function legacyShape(record) {
  const { city, address, road, direction, speed_limit, lat, lng, source, fetched_at } = record;
  return { city, address, road, direction, speed_limit, lat, lng, source, fetched_at };
}
...
test('parseTaipei：Big5 編碼 CSV 正確轉碼與解析（golden）', () => {
  ...
  assert.deepEqual(result.map(legacyShape), [
    {
      city: '臺北市',
      address: '承德路3段 敦煌路口',
      road: '承德路3段',
...
test('parseNewTaipei：固定點解析，區間資料改由專用起訖來源接管（golden）', () => {
...
test('parseNewTaipeiSections：雙向區間展開為兩筆，保留各自起訖座標與長度', () => {
  ...
  const result = parseNewTaipeiSections(csv, FIXED_NOW);

  assert.equal(result.length, 2);
  assert.deepEqual(result.map((record) => record.speed_status), ['confirmed', 'confirmed']);
  assert.deepEqual(result.map((record) => record.camera_type), ['section', 'section']);
  assert.deepEqual(result.map((record) => record.direction), ['往新店方向', '往宜蘭方向']);
  assert.deepEqual(result.map((record) => record.section_length_m), [3946.7, 3938.1]);
  assert.equal(result[0].section_start_lat, 24.9569297);
  assert.equal(result[0].section_end_lng, 121.5981891);
  assert.equal(result[1].section_start_lng, 121.5981016);
  assert.equal(result[1].section_end_lat, 24.9570178);
});

test('parseTaichungMobile：官方混合換行、末欄缺引號與欄名錯置仍正確解析', () => {
  const csv = Buffer.from(
    '\uFEFF\uFEFF編號,轄區分局,行政區,取締地點,取締行向,速限Speed,經度,緯度\n' +
      '"1","第一分局","西區","民權路近中華西街口","西往東","50","24.14342495","120.6737215'
  );
  const [record] = parseTaichungMobile(csv, FIXED_NOW);

  assert.equal(record.speed_status, 'confirmed');
  assert.equal(record.camera_type, 'mobile');
  assert.equal(record.lat, 24.14342495);
  assert.equal(record.lng, 120.6737215);
});
...
test('parseTaichung：文字型 PDF（非掃描），跨頁抽取、備註換行、座標欄位皆正確解析（golden）', async () => {
  ...
  assert.deepEqual(legacyShape(result[0]), {
    city: '臺中市',
  ...
  assert.deepEqual(legacyShape(result[1]), {
    city: '臺中市',
  ...
  assert.deepEqual(legacyShape(result[14]), {
    city: '臺中市',
  ...
  assert.deepEqual(legacyShape(result[30]), {
    city: '臺中市',
