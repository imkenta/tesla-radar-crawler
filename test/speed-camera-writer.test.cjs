...
test('collapseSourceConflictDuplicates：30m 內同 key 合併，並維持第一次出現的 key 順序', () => {
  const first = { source: 'taichung-mobile', address: 'A', direction: '北往南', speed_limit: 50, lat: 24.1, lng: 120.6 };
  const unrelated = { source: 'taichung-mobile', address: 'B', direction: '南往北', speed_limit: 60, lat: 24.2, lng: 120.7 };
  const duplicate = { ...first, lat: 24.10001, lng: 120.60001 };

  const result = collapseSourceConflictDuplicates([first, unrelated, duplicate], 30);

  assert.equal(result.duplicateCount, 1);
  assert.equal(result.speedLimitConflictCount, 0);
  assert.deepEqual(result.records.map((record) => record.address), ['A', 'B']);
});

test('collapseSourceConflictDuplicates：速限衝突採較低正整數值', () => {
  const result = collapseSourceConflictDuplicates([
    { source: 'taichung-mobile', address: 'A', direction: '南北向', speed_limit: 60, lat: 24.1, lng: 120.6 },
    { source: 'taichung-mobile', address: 'A', direction: '南北向', speed_limit: 50, lat: null, lng: null },
  ], 30);

  assert.equal(result.records.length, 1);
  assert.equal(result.records[0].speed_limit, 50);
  assert.equal(result.records[0].lat, 24.1);
  assert.equal(result.records[0].lng, 120.6);
  assert.equal(result.speedLimitConflictCount, 1);
});

test('collapseSourceConflictDuplicates：同 key 但任兩點距離超過 30m 時 fail closed', () => {
  assert.throws(
    () => collapseSourceConflictDuplicates([
      { source: 'taichung-mobile', address: 'A', direction: '南北向', speed_limit: 50, lat: 24.1, lng: 120.6 },
      { source: 'taichung-mobile', address: 'A', direction: '南北向', speed_limit: 50, lat: 24.101, lng: 120.6 },
    ], 30),
    /座標距離超過 30m/
  );
});
