...
test('writeAll：四個 source 全部成功 → upsert 帶正確 onConflict、批次時間戳一致', async (t) => {
  ...
  assert.equal(summary.sourceResults.length, 4);
  ...
  assert.equal(calls.delete.length, 4);
  const deletedSourceNames = calls.delete.map((c) => c.eq.source).sort();
  assert.deepEqual(deletedSourceNames, ['kaohsiung', 'new-taipei', 'taipei', 'taoyuan']);
