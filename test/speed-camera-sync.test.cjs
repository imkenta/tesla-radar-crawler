...
test('writeAll：六個 source 全部成功 → upsert 帶正確 onConflict、批次時間戳一致', async (t) => {
  ...
  const supabase = { from };
  const { geocoder } = fakeGeocoder({ '中西區 中華西路與府前路口路口多功能違規科技執法系統': { lat: 23.0, lng: 120.2 } });

  const summary = await writeAll(supabase, { geocoder });

  assert.equal(summary.sourceResults.length, 6);
  assert.ok(summary.sourceResults.every((r) => r.ok === true));
  assert.deepEqual(
    summary.sourceResults.map((r) => r.name),
    ['taipei', 'new-taipei', 'kaohsiung', 'taoyuan', 'tainan', 'taichung']
  );
  assert.deepEqual(
    summary.sourceResults.map((r) => r.written),
    [1, 1, 1, 1, 1, 31] // taichung fixture 為官方 PDF 前 2 頁節錄，共 31 筆
  );
  ...
  assert.equal(calls.delete.length, 6);
  const deletedSourceNames = calls.delete.map((c) => c.eq.source).sort();
  assert.deepEqual(deletedSourceNames, ['kaohsiung', 'new-taipei', 'taichung', 'tainan', 'taipei', 'taoyuan']);
