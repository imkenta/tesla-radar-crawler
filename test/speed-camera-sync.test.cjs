...
test('writeAll：七個 source 全部成功 → upsert 帶正確 onConflict、批次時間戳一致；全國集執行期聯集去重正確運作', async (t) => {
  ...
  assert.equal(summary.sourceResults.length, 7);
  assert.ok(summary.sourceResults.every((r) => r.ok === true));
  assert.deepEqual(
    summary.sourceResults.map((r) => r.name),
    ['taipei', 'new-taipei', 'kaohsiung', 'taoyuan', 'tainan', 'taichung', 'national-npa']
  );
  assert.deepEqual(
    summary.sourceResults.map((r) => r.written),
    [1, 1, 1, 1, 1, 31, 1] // national-npa：2 筆解析出，1 筆與臺北市重疊被去重丟棄，保留 1 筆（金門縣）
  );

  assert.equal(calls.upsert.length, 7);
  ...
  assert.equal(calls.delete.length, 7);
