...
test('writeAll：五個 source 全部成功 → upsert 帶正確 onConflict、批次時間戳一致', async (t) => {
  ...
  assert.equal(summary.sourceResults.length, 5);
  assert.ok(summary.sourceResults.every((r) => r.ok === true));
  assert.deepEqual(
    summary.sourceResults.map((r) => r.name),
    ['taipei', 'new-taipei', 'kaohsiung', 'taoyuan', 'tainan']
  );
  assert.deepEqual(
  ...
  assert.equal(calls.delete.length, 5);
  const deletedSourceNames = calls.delete.map((c) => c.eq.source).sort();
  assert.deepEqual(deletedSourceNames, ['kaohsiung', 'new-taipei', 'tainan', 'taipei', 'taoyuan']);
