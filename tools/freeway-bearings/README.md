...
3. `3-compute-bearings.cjs`：
   ...
   - **2026-09-26 修正**：挑 OSM 車道的參考方向改成「局部鄰桿方向」（里程差 0.5–3km 的鄰桿連線，兩個方向的桿都算），
     容許 <90°（只要求同側）；沒有局部鄰桿才退回上面那個整段平均（<60°）。原因：整段平均在會轉彎的國道上沒有意義——
     國四西段東西向、東段轉南北向，平均出 131°，用它挑到了**對向車道**，國四東向 24.778K 被寫成 102°（實際 ≈280°），
     2026-09-25 實車整支漏報（TeslaToolbox C-64）。改後重算，現行表 170 筆只有這一筆改變（102→281）。
     輸出最後會列出「與局部鄰桿方向差 ≥90°」的桿：那些必定算反，不得寫進表（`test/freeway-bearing-overrides.test.cjs` 也有同樣的守門）。
