...
// 縣市名稱格式判斷：合法臺灣縣市名稱恆以「市」或「縣」結尾。全國集的 CityName 欄位
// 除 21 個縣市外，還混入「國道一號」「國道3甲」「台2已線」等國道/公路路段分類
// （169 筆，非縣市，本次任務範圍是縣市測速點，不含國道路段），這類值結尾不是市/縣，
// 用此規則即可排除，不需要另外維護國道路名黑名單。
//
// ⚠️ 六都不在此處排除（R9 修正，見 docs/speed-camera-sources.md）：原規格曾打算靜態排除
// 六都（理由：自建源總筆數 1053 > 全國集同範圍 864），但逐都 haversine 30m 重疊分析發現
// 六都各自的「全國集獨有點位」並不少（臺中 80、高雄 47、桃園 35 筆等，臺南自建源甚至
// 完全無原生座標故 0 筆可比對），總量比較無法反映逐點覆蓋率，靜態排除會漏收這些獨有點位。
// 改為：parser 只排除國道等非縣市格式，六都與否一律保留，交由 speed-camera-sync.cjs
// 的執行期聯集去重（見 lib/speed-camera-writer.cjs dedupeAgainstExisting）在寫入階段
// 逐筆比對六個自建源，只丟棄與既有點位座標重疊（≤30m）的筆數。
const CITY_NAME_RE = /(市|縣)$/;

/**
 * 全國集 CityName 是否為合法縣市格式（非國道/公路路段分類）。
 * @param {string} cityName
 * @returns {boolean}
 */
function isEligibleNationalNpaCity(cityName) {
  const trimmed = (cityName || '').trim();
  if (!trimmed) return false;
  return CITY_NAME_RE.test(trimmed);
}
 ...
 * 未來來源若增刪列不會誤刪真實資料）。
 *
 * 本 parser 只過濾非縣市格式的國道/公路路段分類（見 isEligibleNationalNpaCity），
 * **不**在此排除六都——六都與六都以外縣市的重複收錄問題改由 speed-camera-sync.cjs
 * 的執行期聯集去重處理（座標 haversine ≤30m 視為同一支才丟棄），理由見上方註解。
 * 實測全量 1867 筆（不含表頭說明列）扣除國道等 169 筆非縣市格式後，輸出 21 縣市共 1698 筆
 * （六都 864 筆 + 六都以外 15 縣市 834 筆）。
 *
...
function parseNationalNpa(buffer, fetchedAt = new Date().toISOString()) {
  ...
  for (const row of rows) {
    ...
    if (!isEligibleNationalNpaCity(city)) continue;
