...
function toUpsertPayload(record, updatedAt) {
  return {
    ...
    lat: record.lat ?? null,
    lng: record.lng ?? null,
    speed_status: record.speed_status ?? 'unknown',
    enforcement_items_raw: record.enforcement_items_raw ?? null,
    equipment_type_raw: record.equipment_type_raw ?? null,
    classification_basis: record.classification_basis ?? null,
    installation_class: record.installation_class ?? 'unknown',
    speed_measurement_mode: record.speed_measurement_mode ?? 'unknown',
    sensor_technology: record.sensor_technology ?? 'unknown',
    taxonomy_basis: record.taxonomy_basis ?? 'insufficient_evidence',
    taxonomy_source_url: record.taxonomy_source_url ?? null,
    taxonomy_observed_at: record.taxonomy_observed_at ?? null,
    camera_type: record.camera_type ?? 'unknown',
    road_class: record.road_class ?? 'unknown',
    road_level: record.road_level ?? 'unknown',
    direction_mode: record.direction_mode ?? 'unknown',
    direction_bearing: record.direction_bearing ?? null,
    camera_elevation_m: record.camera_elevation_m ?? null,
    section_start_lat: record.section_start_lat ?? null,
    section_start_lng: record.section_start_lng ?? null,
    section_end_lat: record.section_end_lat ?? null,
    section_end_lng: record.section_end_lng ?? null,
    section_length_m: record.section_length_m ?? null,
...
/**
 * 合併同一來源內會撞上 DB unique key (source,address,direction) 的重複紀錄。
 *
 * 只有同 key 的所有已知座標彼此都在 thresholdMeters 內才允許合併；若距離超過門檻，
 * 代表來源把不同點位錯用同一文字 key，直接拋錯，避免靜默覆蓋。速限衝突時採較低的
 * 正整數速限，作為駕駛警示的保守值。輸出順序依第一次出現的 key 保持穩定。
 *
 * @param {object[]} records
 * @param {number} thresholdMeters
 * @returns {{ records: object[], duplicateCount: number, speedLimitConflictCount: number }}
 */
function collapseSourceConflictDuplicates(records, thresholdMeters = 30) {
  const entriesByKey = new Map();
  let duplicateCount = 0;
  let speedLimitConflictCount = 0;

  const validSpeedLimit = (value) =>
    Number.isInteger(value) && value > 0 ? value : null;
  const hasCoordinates = (record) => record.lat != null && record.lng != null;

  for (const record of records) {
    const key = JSON.stringify([record.source, record.address, record.direction ?? '']);
    const existing = entriesByKey.get(key);
    if (!existing) {
      entriesByKey.set(key, {
        record,
        coordinates: hasCoordinates(record) ? [{ lat: record.lat, lng: record.lng }] : [],
      });
      continue;
    }
    ...
    if (hasCoordinates(record)) {
      const outsideThreshold = existing.coordinates.some(
        (point) =>
          haversineMeters(record.lat, record.lng, point.lat, point.lng) > thresholdMeters
      );
      if (outsideThreshold) {
        throw new Error(
          `同來源重複 key 的座標距離超過 ${thresholdMeters}m：` +
            `${record.source} / ${record.address} / ${record.direction ?? ''}`
        );
      }
      existing.coordinates.push({ lat: record.lat, lng: record.lng });
    }

    const existingSpeed = validSpeedLimit(existing.record.speed_limit);
    const incomingSpeed = validSpeedLimit(record.speed_limit);
    if (existingSpeed != null && incomingSpeed != null && existingSpeed !== incomingSpeed) {
      speedLimitConflictCount++;
    }
    const mergedSpeed =
      existingSpeed == null
        ? incomingSpeed
        : incomingSpeed == null
          ? existingSpeed
          : Math.min(existingSpeed, incomingSpeed);
    const preferIncoming =
      (incomingSpeed != null && (existingSpeed == null || incomingSpeed < existingSpeed)) ||
      (!hasCoordinates(existing.record) && hasCoordinates(record));

    const preferredRecord = preferIncoming ? record : existing.record;
    const coordinateRecord = hasCoordinates(preferredRecord)
      ? preferredRecord
      : hasCoordinates(existing.record)
        ? existing.record
    ...
    existing.record = {
      ...preferredRecord,
      ...(coordinateRecord ? { lat: coordinateRecord.lat, lng: coordinateRecord.lng } : {}),
      speed_limit: mergedSpeed,
    };
  }

  return {
    records: Array.from(entriesByKey.values(), (entry) => entry.record),
    duplicateCount,
    speedLimitConflictCount,
  };
}
