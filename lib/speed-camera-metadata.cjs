...
function inferRoadClass(record) {
  ...
  // national-npa 把國道路段名塞進 CityName（如「國道五號」），但 freeway-npa
  // （data.gov.tw/dataset/13940）縣市／行政區欄位固定留白，路名只出現在
  // 「設置地點」（此處的 road 欄，例：「國道五號北向16.9公里」）。兩來源都要辨識，
  // 否則同一條國道換來源時會忽明忽暗判成 unknown
  // （2026-08-03 雪隧對向測速因 road_class=unknown 被 App 端判斷放行，實車誤報事故）。
  if (/^國道/.test(city) || /^國道/.test(road)) return 'freeway';
