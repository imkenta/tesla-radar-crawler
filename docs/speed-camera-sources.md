...
| 桃園市 | [桃園市固定式測速照相](https://data.gov.tw/dataset/25935) | CSV | 有（經度/緯度） | 不定期 | **Big5**（需轉碼） | 171 筆 | ⬜ 已查證可用，本輪未寫解析器（規格只要求≥2縣市，優先做台北/新北/高雄三個代表性樣本） |
| 高雄市 | [111年固定式違規照相設備及科技執法](https://data.gov.tw/dataset/148455) | CSV/JSON | 有（座標緯N度/座標經E度，非標準欄位命名） | 不定期 | UTF-8 (BOM) | 248 筆 | ✅ |
| 台中市 | 查無結構化開放資料集 | **無** | — | — | — | — | ❌ 查無資料 |
| 台南市 | [智慧管理科技執法設備設置地點](https://data.tainan.gov.tw/Resource/1c7e82f0-d6b2-4b20-aeff-5c768100f82c) | CSV/JSON | **無**（僅文字路段描述） | 最後更新 2025-12-26 | UTF-8 (BOM) | 72 筆 | ⬜ 需 geocode，本輪未實作解析器（留待下一輪） |
...
- 陷阱：Big5 編碼；地址是片段（如「成功路三段235號前」），不含行政區全名，組完整地址需拼接「設置行政區」欄位。
...
- 「設置位置」是複合字串，例：`北門路段(火車站前圓環至青年路)自動辨識違規停車及不依標線行駛科技執法系統【違規停車、違規臨時停車...】`，混雜地點描述與取締項目說明，需要額外清理才能丟給 geocoder，準確度存疑。
- 「行政區」欄位是**行政區代碼**（如 `67000320`）不是文字，需另查代碼表轉換成地名才能組出可 geocode 的地址。
- 本輪未實作台南解析器：地址清理 + 行政區代碼轉換的規則複雜度較高，且測速項目與其他違規項目（違規停車等）混雜在同一系統描述內，需要先定義「什麼算測速照相」的篩選規則，建議留待下一輪單獨處理。
...
本輪實作的 3 縣市（台北/新北/高雄）皆已有原生座標，**不需要 geocode**。`lib/geocoder.cjs` 模組已備妥（供台南等無座標縣市未來使用），節流 1 req/s、帶自訂 User-Agent，測試全部用 mock，不打真實 Nominatim。
